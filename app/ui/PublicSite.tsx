...
export function PublicHeader() {
  ...
  return <header className="site-header">
    ...
    <nav id="public-navigation" className={open ? "open" : ""} aria-label="Primary navigation">
      <Link href="/platform" onClick={() => setOpen(false)}>Platform</Link><Link href="/lms" onClick={() => setOpen(false)}>LMS</Link><Link href="/content-delivery" onClick={() => setOpen(false)}>Content delivery</Link><Link href="/organisations" onClick={() => setOpen(false)}>Organisations</Link><Link href="/catalogue" onClick={() => setOpen(false)}>Catalogue</Link><Link href="/about" onClick={() => setOpen(false)}>Company</Link>
    </nav>
...
export function PublicFooter() {
  return <footer className="site-footer">
    ...
    <div><b>Product</b><Link href="/platform">Platform</Link><Link href="/lms">Learning experience</Link><Link href="/content-delivery">Content delivery</Link><Link href="/organisations">For organisations</Link><Link href="/catalogue">Course catalogue</Link></div>
...
export function PublicPage({ data, children }: { data: PublicPageData; children?: ReactNode }) {
  ...
  useGSAP(() => {
    ...
    media.add("(prefers-reduced-motion: no-preference)", () => {
      gsap.from(".public-hero > div > *, .public-hero aside", { y: 30, autoAlpha: 0, duration: .8, stagger: .08, ease: "power3.out" });
      ScrollTrigger.batch(".public-feature-grid article, .public-scroll-reveal", {
        start: "top 86%",
        once: true,
        onEnter: (items) => gsap.fromTo(items, { y: 46, autoAlpha: 0 }, { y: 0, autoAlpha: 1, duration: .75, stagger: .08, ease: "power3.out", overwrite: true }),
      });
      gsap.from(".public-statement > *", { x: (index) => index ? 36 : -36, autoAlpha: 0, duration: .8, stagger: .1, ease: "power3.out", scrollTrigger: { trigger: ".public-statement", start: "top 78%", once: true } });
    });
    return () => media.revert();
  }, { scope: root });
  return <div ref={root} className={`public-site accent-${data.accent || "purple"}`}><a className="skip-link" href="#main-content">Skip to main content</a><PublicHeader/><main id="main-content">
