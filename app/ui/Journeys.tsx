...
export function CertificateExperience({certificateId}:{certificateId:string}) {
  ...
  return <main className="certificate-page"><header><AtlasMark/><a href="/learn"><X/></a></header><section><div className="certificate-copy"><span className="journey-kicker"><Sparkles/> Achievement unlocked</span><h1>A new proof<br/>of your growth.</h1><p>Your certificate is verified, recorded and ready to share as part of your professional learning history.</p><div className="certificate-actions"><button className="journey-primary" onClick={()=>window.print()}>Download certificate <FileText/></button><button onClick={()=>{navigator.clipboard?.writeText(`https://verify.atlas.demo/${certificateId}`);setShared(true)}}>Share achievement <ArrowRight/></button></div>{shared&&<p className="share-success"><CheckCircle2/> Verification link copied to your clipboard.</p>}<a href="/learn">Return to my learning</a></div><div className="certificate-wrap"><article><div className="certificate-brand"><AtlasMark/><span>FEDERAL SERVICE<br/>LEARNING DIRECTORATE</span></div><small>CERTIFICATE OF COMPLETION</small><p>This certifies that</p><h2>Amara Okafor</h2><p>has successfully completed</p><h3>{credential?.title||course.title}</h3><div className="certificate-meta"><span><b>{credential?.issued||"30 July 2026"}</b><small>Date issued</small></span><span><b>{course.duration}</b><small>Learning time</small></span><span><b>80%</b><small>Final score</small></span></div><div className="certificate-sign"><span><i>{course.instructor.split(" ")[0]}</i><small>{course.instructor}<br/>Course instructor</small></span><strong><Award/></strong><span><i>Ifeoma</i><small>Ifeoma Kalu<br/>Learning director</small></span></div><footer><span>Certificate ID: {certificateId}</span><span>verify.atlas.demo/{certificateId}</span></footer></article></div></section></main>;
}
