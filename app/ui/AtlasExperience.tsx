...
function LearnerWorkspace({active,courses:availableCourses,globalQuery}:{active:string;courses:AtlasCourse[];globalQuery:string}) {
  ...
  if(active==="Pathways") {
    const featuredPaths=[cataloguePaths[7],cataloguePaths[20],cataloguePaths[9],cataloguePaths[13],cataloguePaths[24],cataloguePaths[4]];
    return <section className="library-view"><div className="library-hero pathway-hero"><span className="kicker">25 catalogue pathways</span><h2>Your pathways</h2><p>Follow the official Atlas catalogue from foundation through advanced workplace capability.</p></div><div className="pathway-list scroll-reveal">{featuredPaths.map((path,i)=>{const first=availableCourses.find(course=>course.code===path.courses[0]?.code);const progress=i===0?50:i===1?24:0;return <article key={path.code} style={{background:["#dcd5ff","#ffd9e8","#ffe6ad","#d5f7d9","#cceffc","#f2dcff"][i]}}><span>{path.code}</span><div><small>{progress?"IN PROGRESS":"RECOMMENDED FOR YOUR ROLE"}</small><h3>{path.title}</h3><p>{path.courses.slice(0,3).map(course=>course.title).join(" · ")}</p><div className="progress"><i style={{width:`${progress}%`}}/></div><b>{progress?`${Math.max(1,Math.round(path.courses.length*progress/100))} of ${path.courses.length}`:`${path.courses.length}`} courses · {path.courses.reduce((sum,course)=>sum+course.lessons.length,0)} lessons</b></div><button disabled={!first} onClick={()=>first&&(window.location.href=`/learn/course/${first.id}`)}>{progress?"Continue":"Start pathway"} <ArrowRight/></button></article>})}</div></section>;
  }
...
function CMS() {
  ...
  return <div ref={root} className="app-shell cms-shell">
    <aside className="app-sidebar cms-sidebar"><Brand light/><button className="create-button" onClick={()=>setCreate(true)}><Plus size={18}/> Create new <ChevronDown size={15}/></button>
      <nav>{cmsNav.map(group=><div key={group.group}><small>{group.group}</small>{group.items.map(([label,Icon])=><button key={label as string} className={active===label?"active":""} onClick={()=>setActive(label as string)}><Icon size={18}/><span>{label as string}</span>{label==="Courses"&&<em>{atlas.courses.length}</em>}</button>)}</div>)}</nav>
    ...
    <main className="app-main">
      ...
      <div className="cms-content">
        {active==="Dashboard" ? <CMSDashboard onCreate={()=>setCreate(true)} published={published} onNavigate={setActive}/> : <CMSSection active={active} onCreate={()=>setCreate(true)} published={published} catalogue={atlas.courses}/>}
      </div>
    </main>
    {create&&<CourseModal onClose={()=>setCreate(false)} onSaveDraft={(course)=>{saveCourse({...course,published:false,lifecycle:"Draft"});setCreate(false)}} onPublish={(course)=>{publishCourse(course);setPublished(course);setCreate(false)}}/>}
  </div>;
}
...
function CMSSection({active,onCreate,published,catalogue}:{active:string;onCreate:()=>void;published:AtlasCourse|null;catalogue:AtlasCourse[]}) {
  ...
  if(!isCourses) return <CMSFunctionalSection active={active} notify={act}/>;
